...
> **30 秒速读**：Neovim 配置在 `~/.config/nvim/init.lua`，用 `vim.opt` 设选项、`vim.keymap.set` 设快捷键，配置可拆到 `lua/config/` 多个文件。
> 
> **如果只记一件事**：用 `vim.opt` 而不是 `vim.o` 设选项，它会智能处理全局和局部作用域。
...
| 错误 | 症状 | 解决 |
|------|------|------|
| `:q` 关了窗口以为文件没了 | 关掉分屏后文件"消失" | `:q` 只关窗口，`:bd` 才关缓冲区；用 `:ls` 查看所有缓冲区 |
| Ctrl-w 方向键分屏切换不生效 | 按了 Ctrl-w 但窗口没切换 | Ctrl-w 和 hjkl 是两步：先按 Ctrl-w 松开，再按 h/j/k/l |
| 写了 init.lua 但 Neovim 没加载 | 配置项不生效，还是默认行为 | 检查路径：Linux/macOS 必须在 `~/.config/nvim/init.lua` |
| `vim.o.number = true` 分屏后新窗口没行号 | 只有第一个窗口有行号 | 改用 `vim.opt.number = true`，它同时设全局和当前窗口 |
| 标签页开了一堆，以为是多文件编辑 | 每个标签页一个文件，切换很乱 | 标签页是窗口布局集合，多文件用缓冲区 `:bn`/`:bp` 切换 |
