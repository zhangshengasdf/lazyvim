...
> 你的 Neovim 启动要 500ms？每次打开大文件卡 2 秒？LSP 莫名其妙罢工？
> 本章教你用 `:checkhealth`、`:Lazy profile`、`:Lazy debug` 三板斧定位问题，
> 然后用懒加载优化、禁用内置插件、调整 `vim.opt` 三招把启动压到 50ms 以内。
> 这是教程的收尾章，帮你把前 19 章的配置打磨成生产级状态。
